...
int do_command(krb5_context context, krb5_keytab keytab, 
		 krb5_principal me, char *princ, 
		 char *cmd, char *cmddir)
{
	char	*p;
	char    *answer;
	static  char    answer_exec[]   = 
			"Cannot execute command.";
	static  char    answer_priv[]   = 
			"You are not privileged to execute this command.";
	static  char    answer_regexp[] = 
	...
	*p = '\0';
	...
	else
	{
		...
		pathenv = (char *)malloc( (strlen(cmddir) + 6) * sizeof(char));
		if(pathenv == NULL)
		{
		...
		{
			syslog(LOG_ERR, "%s while resolving ccache",
						error_message(retval));
			exit(1);

		}
#ifdef __osf__
                sprintf(ccname, "KRB5CCNAME=FILE:/tmp/afsadm_%d", 
					getpid());
#else

		snprintf(ccname, 255, "KRB5CCNAME=FILE:/tmp/afsadm_%d",
					 getpid());
#endif
		...
		{
			syslog(LOG_ERR, "%s while initialize ccache",
					error_message(retval));
		...
		memset((char *)&creds, 0, sizeof(creds));
    		...
    		if((retval = krb5_build_principal_ext(
				context,
				&tgtserver,
				...
				krb5_princ_realm(context, me)->data,
                                0))) 
		{
              		syslog(LOG_ERR, "%s while building server name", 
				error_message(retval));
		...
		if(retval = krb5_get_in_tkt_with_keytab(context,
                                			0, 
                                			NULL,
                                			NULL,
                                			preauth,
                                			...
                                			0 ))
		{
			syslog(LOG_ERR, "%s while getting tgt", 
						error_message(retval));
			krb5_cc_destroy(context, ccache);
			exit(1);
		} 
		...
		localcmd = (char *)malloc(sizeof(char) * 
					 (strlen(cmd) + strlen(cmddir) + 2));
		if(localcmd == NULL)
		{
			syslog(LOG_ERR, 
			       "Not enough memory (cmdpath malloc)");
			exit(1);
		}
		sprintf(localcmd, "%s/%s", cmddir, cmd);

		syslog(LOG_INFO, "Principal %s : system(%s)",
			 		princ, localcmd);
		...
		syslog(LOG_INFO, "Principal %s : system(%s) returns with %d", 
					princ, localcmd, result);

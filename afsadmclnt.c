 ...
 **********************************************************************/
int client_receive_reply(krb5_context context, krb5_auth_context auth_context, int sock) {
	char buff[BUFFSIZE];
	short netlen;
	...
	for (;;) {
		...
		if ((len = recv(sock, (char *)&netlen, sizeof(netlen), 0)) < 0)
			com_err(progname, errno, "while receiving len of encrypted message");
		...
		if (reclen > bufflen) {
			fprintf(stderr, "Cannot receive all encrypted data (small buffer)\n");
			return 1;
		}
		...
		if (len != reclen) {
			fprintf(stderr, "Received only %d bytes of encrypted message (expected %d bytes)\n", reclen, len);
			return 1;
		}
		...
		if (len > 0) {
			...
			/* Decrypt data */
			if (retval = krb5_rd_priv(context, auth_context, &kdata, &packet, NULL)) {
	...
	printf("\n");
	if (debug)
		fprintf(stderr, "End of stdout/stderr [Total: %d/%d bytes (encr/decr) in %ds]\n", encdatalen, datalen, (int)(endtime - starttime));
...
char* connect_to_server(char *host, int port, int *retsock) {
	...
	if (port != 0) {
		...
		if (!sp) {
			fprintf(stderr, "unknown service %s/tcp; check /etc/services or /etc/nsswitch.conf \n", AFSADM_PORT);
			return NULL;
		}
	...
	memcpy((char *)&s_saddr.sin_addr, (char *)hp->h_addr, sizeof(hp->h_addr));
...
int client_init_and_authenticate(krb5_context *context, krb5_auth_context *auth_context, krb5_rcache *rcache, char *dnsname, char *serv, int sock) {
	...
	if (retval = krb5_sname_to_principal(*context, dnsname, service, KRB5_NT_SRV_HST, &server)) {
		com_err(progname, retval, "while creating server name for %s/%s", admhost, service);
		return 1;
	}
	...
	if (retval = krb5_auth_con_setports(*context, *auth_context, &ckaddr, &skaddr)) {
	...
	if (retval = krb5_auth_con_setaddrs(*context, *auth_context, &ckaddr, &skaddr)) {
 ...
 *
 ***********************************************************************/
int client_send_request(krb5_context context, krb5_auth_context auth_context, int sock, char *request) {
	...
	if (retval = krb5_mk_priv(context, auth_context, &data, &kdata, NULL)) {

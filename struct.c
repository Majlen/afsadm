...
int create_group(char *name)
{
	...
	if( (gp = (struct group *)malloc(sizeof(struct group))) == NULL )
		return(F_NOMEM);

	if((gp->grpname = (char *)malloc(sizeof(char) * (strlen(name) + 1)) ) == NULL)
		return(F_NOMEM);
...
int add_to_group(char *membername)
{
	...
	if((mem = (struct member *)malloc(sizeof(struct member))) == NULL)
		return(F_NOMEM);	
	
	if((mem->memname = (char *)malloc( 
			(strlen(membername) + 1) * sizeof(char) )) == NULL)
		return(F_NOMEM);
...
int create_command(char *cmdid)
{
	...
	if( (cp = (struct command *)malloc(sizeof(struct command))) == NULL )
		return(F_NOMEM);

	if((cp->cmdid = (char *)malloc(sizeof(char) * (strlen(cmdid) + 1)) ) == NULL)
		return(F_NOMEM);
...
int add_regexp_to_comm(char *grp, char *regexp)
{
	...
	if((actcmd->grptable = (struct group **)realloc(actcmd->grptable, 	
		sizeof(struct group *) * (actcmd->grpnum + 1))) == NULL)
		return(F_NOMEM);

	if((actcmd->regexptable = (char **)realloc(actcmd->regexptable,
		sizeof(char *) * (actcmd->grpnum + 1))) == NULL)
		return(F_NOMEM);

	if((pr = (char *)malloc((strlen(regexp) + 1) * sizeof(char) )) == NULL)
		return(F_NOMEM);
...
int add_to_cmd_list(char *cmd)
{
	...
	if( strlen(cmd) > 0 )
	{
	  if((actcmd->list = (char *)malloc(
			sizeof(char) * (strlen(cmd) + 1))) == NULL)
		return(F_NOMEM);
...
int add_to_cmd_help(char *hlp)
{
	if(actcmd == NULL)
		return(F_NOMEM);
	if((actcmd->help = (char *)malloc(sizeof(char) * (strlen(hlp) + 1))) == NULL)
		return(F_NOMEM);
...
char *get_list_of_groups(void)
{
	...
	for(pg = grp_table; pg != NULL; pg = pg->nextgrp )
	{
		if(pg->grpname != NULL)
		{
			if(p == NULL)
			{
			  p = (char *)malloc(sizeof(char) * (strlen(pg->grpname) + 2));
			  p[0] = '\0';
			}
			else
			  p = (char *)realloc(p, 
			    sizeof(char) * (strlen(p) + strlen(pg->grpname) + 2));
			if(p == NULL)
				break;
			strcat(p, pg->grpname);
			strcat(p, "\n");
...
char *get_list_of_cmd(void)
{
	...
	for(pc = cmd_table; ; pc = pc->nextcmd )
	{
		...
		if(pc->list != NULL)
		{
			if(p == NULL)
			{
			  p = (char *)malloc(sizeof(char) * (strlen(pc->list) + 2));
			  p[0] = '\0';
			}
			else
			  p = (char *)realloc(p, 
			    sizeof(char) * (strlen(p) + strlen(pc->list) + 2));
			if(p == NULL)
				break;
			strcat(p, pc->list);
			strcat(p, "\n");
...
char *get_help(char *cmd)
{
	...
	for(pc = cmd_table; ; pc = pc->nextcmd )
	{
		...
		if(	(pc->list != NULL) && 
			(pc->help != NULL) && 
			(strcmp(cmd, pc->list) == 0) )
		{
			p = (char *)malloc(sizeof(char) * (strlen(pc->help) + 1));
			if(p != NULL)
				strcpy(p, pc->help);
			break;
		}
...
#ifdef _DEBUG_
				syslog(LOG_DEBUG,
					"Debug: nested test for membership");
#endif 
				if((pgrp = find_grp(pmem->memname)) == NULL)
				{
					syslog(LOG_DEBUG, 
					  "err: grp %s is not in grouptable!\n",
					   pmem->memname);
					return(CHK_NOGRP);
				}
				else
